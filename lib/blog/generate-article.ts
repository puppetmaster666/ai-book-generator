...
async function pickTopic(): Promise<TopicTemplate> {
  ...
  // Get the last 3 post keywords to avoid repeating the same category
  const recentKeywords = existingPosts.slice(0, 3).map(p => p.primaryKeyword);
  ...
  // All keyword categories we want to rotate through
  const allKeywords = [...new Set(TOPIC_POOL.map(t => t.primaryKeyword))];

  if (unusedTopics.length === 0) {
    // All topics used. Rotate through keywords, skipping recent ones.
    const sortedKeywords = allKeywords
      .map(kw => ({ kw, count: keywordCounts[kw] || 0, recent: recentKeywords.includes(kw) }))
      .filter(k => !k.recent) // Skip keywords used in last 3 posts
      .sort((a, b) => a.count - b.count);

    const nextKeyword = sortedKeywords[0]?.kw || allKeywords[Math.floor(Math.random() * allKeywords.length)];

    const angles = [
      'fresh perspective with new insights and practical examples',
      'advanced tips for experienced users',
      'beginner mistakes and how to avoid them',
      'comparison with manual/traditional approaches',
      'creative use cases most people overlook',
      'step-by-step tutorial with a specific project',
    ];

    return {
      primaryKeyword: nextKeyword,
  ...
  // Filter out topics whose keyword was used in the last 3 posts
  const freshTopics = unusedTopics.filter(t => !recentKeywords.includes(t.primaryKeyword));
  const candidates = freshTopics.length > 0 ? freshTopics : unusedTopics;
